...
public class GoodsController extends BaseController {
	...
	public String list(int uid, HttpServletRequest req) {
		...
		if ("admin".equals(userMapper.selectById(uid).username)) {
			req.setAttribute("retList", goodsMapper.selectAll());
		} else {
			req.setAttribute("retList", goodsMapper.selectByUid(uid));
		}
		return "/goods/list"; // 转发，可以打开templates/activity/list.html
	}
	// 请求分2种
	// 1. GET请求
	//	地址栏可见，不太安全
	...
	public String add(int uid, Integer id, HttpServletRequest req) {
		...
		req.setAttribute("bean", id==null?null:goodsMapper.selectById(id));
		return "/goods/add"; // templates/activity/add.html
	}
	// <form action="" method="post"></form>
	@PostMapping("/add")
	public String add(GoodsBean bean, HttpServletResponse resp) {
		if (NotNullUtil.isBlank(bean)) {
			return jsAlert("请完善信息！", 
				"/goods/add?uid="+bean.uid+(bean.id==null?"":"&id="+bean.id), 
					resp);
		}
		if (bean.id == null) { // 添加活动
			goodsMapper.insert(bean);
		} else { // 修改活动
			goodsMapper.updateById(bean);
		}
		return "redirect:/goods/list?uid="+bean.uid;
	}
	...
	public String del(int uid, int id) {
		goodsMapper.deleteById(id); // 按照主键id删除这条数据
		joinMapper.deleteJoinByAid(id);
		return "redirect:/goods/list?uid="+uid; // 删除成功，重新刷新列表查询页面
	}
